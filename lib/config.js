...
export const DEFAULT_CONFIG = {
  _readme:
    ...
    'HARNESS_INSTRUCTIONS, HARNESS_MAX_STEPS.',
  openai: {
...
function applyEnvOverrides(cfg) {
  ...
  if (process.env.HARNESS_MAX_STEPS) cfg.maxSteps = Number(process.env.HARNESS_MAX_STEPS);
}
...
function validate(cfg) {
  ...
  cfg.maxSteps =
    Number.isFinite(Number(cfg.maxSteps)) && Number(cfg.maxSteps) >= 1
      ? Math.min(1000, Math.floor(Number(cfg.maxSteps)))
  ...
  cfg.searchIgnore = (Array.isArray(cfg.searchIgnore) ? cfg.searchIgnore : [])
    .map((v) => String(v ?? '').trim())
    .filter(Boolean);
  cfg.instructions =
