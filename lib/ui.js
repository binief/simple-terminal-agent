...
/** Per-call token/speed footnote printed under an assistant reply. */
export function printUsage({ prompt, completion, tokPerSec, elapsedMs, session, contextPct, estimated }) {
  const bits = [];
  bits.push(`${fmtTokens(prompt)} in`);
  bits.push(`${fmtTokens(completion)} out`);
  if (tokPerSec != null) bits.push(`${tokPerSec < 10 ? tokPerSec.toFixed(1) : Math.round(tokPerSec)} tok/s`);
  bits.push(fmtDuration(elapsedMs));
  if (session != null) bits.push(`session ${fmtTokens(session)}`);
  if (contextPct != null) bits.push(`context ${Math.min(999, Math.round(contextPct))}%`);
  const line = bits.join(' · ');
  out(`  ${s.blue('ⓘ')} ${s.dim(`${estimated ? '~' : ''}${line}`)}`);
}

/** Session summary for /usage. */
export function printStats({ calls, promptTokens, completionTokens, avgTokPerSec, compactions, used, contextSize, estimated }) {
  const total = promptTokens + completionTokens;
  out(
    `  ${s.blue('ⓘ')} ${s.dim(
      `session ${calls} call(s) · ${fmtTokens(promptTokens)} in · ${fmtTokens(completionTokens)} out · ${fmtTokens(total)} tokens` +
        (avgTokPerSec ? ` · avg ${avgTokPerSec < 10 ? avgTokPerSec.toFixed(1) : Math.round(avgTokPerSec)} tok/s` : '') +
        (estimated ? ' (partly estimated)' : '')
    )}`
  );
  const pct = contextSize ? Math.min(999, Math.round((used / contextSize) * 100)) : null;
  out(
    `  ${s.blue('ⓘ')} ${s.dim(
      `context ${fmtTokens(used)}${contextSize ? `/${fmtTokens(contextSize)}` : ''}${pct != null ? ` (${pct}%)` : ''} · ${compactions} compaction(s)`
    )}`
  );
}
...
/**
 * Progress indicator for a long-running step (model call, tool run, …).
 * - TTY: animated frame + live elapsed seconds, erased on stop.
 * - Piped/NO_COLOR: a single "· label" line so logs still show progress.
 * Returns { stop(), elapsedMs() }; stop() is safe to call more than once.
 */
export function spinner(label, { showElapsed = true } = {}) {
  ...
  const draw = () => {
    ...
    process.stdout.write(`\r\x1b[K${s.dim(`${frames[i++ % frames.length]} ${label}${showElapsed ? ` ${secs}s` : ''}`)}`);
  };
  draw();
