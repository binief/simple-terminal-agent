...
const DEFAULT_READ_LINES = 200; // first look is a peek — search_files for the rest
const MAX_READ_LINES = 500; // hard per-call cap: whole-file dumps are not possible
const MAX_READ_CHARS = 40_000; // per-result budget: minified/long lines cannot slip past the line cap
  ...
  const defs = [
    {
      name: 'read_file',
      description:
        'Read a text file from the workspace as a line window (default 200 lines, at most 500 lines and 40k chars per call). ' +
      ...
      parameters: {
        ...
        properties: {
          ...
          limit: { type: 'integer', description: 'Max lines to return, capped at 500 per call. Default 200. Page with offset using the continuation hint.' },
        },
        required: ['path'],
      },
